(ns gong.main
  "Kotodama WASM entrypoint for the Gong clean-room actor (L4) — Clojure port.
  ...
  facts (`gong.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"gong.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gong-compat" :tier tier :entities entities} 200])
